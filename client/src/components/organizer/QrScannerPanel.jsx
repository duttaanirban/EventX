...
export function QrScannerPanel() {
  ...
  return (
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(20rem,.85fr)]">
      <section className="rounded-xl border border-white/[0.08] bg-[#101720] p-4 shadow-[0_12px_34px_rgba(0,0,0,0.16)] sm:p-5" aria-labelledby="qr-scanner-title">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 id="qr-scanner-title" className="text-base font-semibold text-slate-100">Camera scanner</h2>
        ...
        <div className="mt-5 overflow-hidden rounded-xl border border-white/[0.08] bg-[#080d14] p-3">
          {isScannerOpen ? (
            <div id="eventx-qr-scanner" className="min-h-80 overflow-hidden rounded-lg text-slate-200" />
          ) : (
            <div className="grid min-h-80 place-items-center px-5 text-center">
              <div>
                <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-teal-300/10 bg-teal-400/10 text-teal-300">
                  <QrCode weight="duotone" aria-hidden="true" className="h-7 w-7" />
                </span>
                <p className="mt-4 text-sm font-semibold text-slate-200">Scanner ready</p>
      ...
      <div className="space-y-4">
        <section className="rounded-xl border border-white/[0.08] bg-[#101720] p-4 shadow-[0_12px_34px_rgba(0,0,0,0.16)] sm:p-5" aria-labelledby="manual-validation-title">
          <h2 id="manual-validation-title" className="text-base font-semibold text-slate-100">Manual validation</h2>
          ...
          <form onSubmit={submitManual} className="mt-4">
            ...
            <textarea
              ...
              className="focus-ring min-h-36 w-full resize-y rounded-xl border border-white/10 bg-white/[0.045] px-3 py-3 font-mono text-xs leading-5 text-white transition placeholder:text-slate-600 hover:border-white/20 focus:border-teal-400/60"
              value={manualPayload}
        ...
        {validationError ? (
          <div role="alert" className="rounded-xl border border-rose-400/20 bg-rose-950/20 p-4 text-rose-100">
            <div className="flex items-start gap-3">
              <WarningCircle weight="duotone" aria-hidden="true" className="mt-0.5 h-5 w-5 shrink-0 text-rose-300" />
              <div>
                <p className="text-sm font-semibold">Validation failed</p>
                <p className="mt-1 text-xs leading-5 text-rose-200/75">{validationError}</p>
              </div>
            </div>
          </div>
        ) : null}
        ...
        {lastCheckin ? (
          <section className="rounded-xl border border-teal-300/15 bg-teal-400/[0.08] p-4" aria-labelledby="last-checkin-title">
            <div className="flex items-start gap-3">
              <CheckCircle weight="duotone" aria-hidden="true" className="h-6 w-6 shrink-0 text-teal-300" />
              <div className="min-w-0">
                <h2 id="last-checkin-title" className="text-sm font-semibold text-teal-100">Check-in successful</h2>
                <p className="mt-2 truncate font-semibold text-white">{lastCheckin.user?.name}</p>
                <p className="mt-0.5 truncate text-sm text-slate-400">{lastCheckin.event?.title}</p>
              </div>
            </div>
          </section>
        ) : null}
