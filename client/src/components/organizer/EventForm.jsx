...
export function EventForm({ event, onSubmit, onCancel, isSubmitting }) {
  ...
  return (
    <form
      ...
      className="rounded-xl border border-white/[0.08] bg-[#101720] p-4 shadow-[0_12px_34px_rgba(0,0,0,0.16)] sm:p-5"
      noValidate
    >
      <div className="flex items-start gap-3 border-b border-white/[0.08] pb-4">
        ...
        <div>
          <h3 className="text-base font-semibold text-slate-100">{event ? 'Edit event' : 'Create event'}</h3>
      ...
      <fieldset className="mt-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="block md:col-span-2">
            ...
            <textarea
              className="focus-ring min-h-28 w-full resize-y rounded-xl border border-white/10 bg-white/[0.045] px-3 py-3 text-sm text-white shadow-sm transition placeholder:text-slate-500 hover:border-white/20 focus:border-teal-400/60"
              aria-invalid={errors.description ? 'true' : undefined}
              {...register('description')}
            />
            {errors.description ? <span role="alert" className="mt-1.5 block text-xs font-medium text-rose-400">{errors.description.message}</span> : null}
          </label>
        </div>
      </fieldset>
      ...
      <fieldset className="mt-6 border-t border-white/[0.08] pt-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <Input tone="dark" label="Date" type="date" className="[color-scheme:dark]" {...register('date')} error={errors.date?.message} />
      ...
      <div className="mt-6 flex flex-col-reverse gap-3 border-t border-white/[0.08] pt-5 sm:flex-row sm:justify-end">
        {onCancel ? (
          <Button type="button" variant="nav" className="border border-white/10" onClick={onCancel}>
            Cancel
          </Button>
        ) : null}
