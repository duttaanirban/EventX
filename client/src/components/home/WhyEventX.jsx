...
export function WhyEventX() {
  return (
    <section aria-labelledby="why-eventx-heading" className="border-t border-slate-200 dark:border-white/[0.06] bg-slate-100 dark:bg-[#0a1018] py-12 text-slate-900 dark:text-white sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl">
          <h2 id="why-eventx-heading" className="text-2xl font-bold">Why EventX</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">Everything you need to create, manage, and scale memorable events.</p>
        </div>
        <div className="mt-7 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {features.map(({ icon: Icon, title, description }) => (
            <article
              key={title}
              className="group relative isolate h-full overflow-hidden rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101923] p-6 transition duration-200 hover:border-teal-400/20 hover:shadow-[0_12px_32px_rgba(0,0,0,0.2)] motion-safe:hover:-translate-y-1"
            >
