...
export function AdminAnalytics() {
    ...
    return (
      <div className="space-y-5" aria-busy="true" aria-label="Loading admin analytics">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Skeleton className="h-44 !rounded-xl !bg-slate-900/5 dark:!bg-white/5" />
          <Skeleton className="h-44 !rounded-xl !bg-slate-900/5 dark:!bg-white/5" />
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720] p-5" aria-labelledby="account-composition-title">
          <h2 id="account-composition-title" className="text-base font-semibold text-slate-900 dark:text-slate-100">Account composition</h2>
          <p className="mt-1 text-xs text-slate-500">Attendee and organizer accounts currently on the platform</p>
          <div className="mt-6 flex h-2.5 overflow-hidden rounded-full bg-slate-900/5 dark:bg-white/[0.06]" aria-hidden="true">
          ...
          <div className="mt-5 grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{data.totalUsers}</p>
            </div>
            <div>
              <p className="flex items-center gap-2 text-xs text-slate-500">
              ...
              <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{data.totalOrganizers}</p>
            </div>
          </div>
        </section>

        <section className="rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720] p-5" aria-labelledby="payment-summary-title">
          <h2 id="payment-summary-title" className="text-base font-semibold text-slate-900 dark:text-slate-100">Payment summary</h2>
          <p className="mt-1 text-xs text-slate-500">Current payment records and revenue from paid transactions</p>
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div className="rounded-lg border border-slate-200 dark:border-white/[0.07] bg-slate-900/5 dark:bg-black/10 p-4">
              <p className="text-xs font-medium text-slate-500">Records</p>
              <p className="mt-2 text-xl font-bold text-slate-900 dark:text-white">{data.totalTransactions}</p>
            </div>
            <div className="rounded-lg border border-slate-200 dark:border-white/[0.07] bg-slate-900/5 dark:bg-black/10 p-4">
              <p className="text-xs font-medium text-slate-500">Paid revenue</p>
              <p className="mt-2 break-words text-xl font-bold text-slate-900 dark:text-white">{formatCurrency(data.platformRevenue)}</p>
            </div>
          </div>
        </section>
      </div>
