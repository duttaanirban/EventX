...
export default function EventDetailPage() {
  ...
  if (isError || !event) {
    ...
    return (
      <main className="min-h-[calc(100dvh-4.5rem)] bg-slate-50 dark:bg-[#080d14] text-slate-900 dark:text-white">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div role="alert" className="mt-8 rounded-2xl border border-rose-400/20 bg-rose-950/20 px-6 py-12 text-center">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-rose-700/80 dark:text-rose-100/80">{message}</p>
          </div>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-[calc(100dvh-4.5rem)] overflow-hidden bg-slate-50 dark:bg-[#080d14] text-slate-900 dark:text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8 lg:px-8">
        ...
        <section className="relative mt-5 aspect-[16/10] min-h-72 overflow-hidden rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#101720] sm:aspect-[16/8] lg:aspect-[16/7]" aria-labelledby="event-title">
          {event.bannerImage ? (
        ...
        <section className="mt-8 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_22rem] xl:grid-cols-[minmax(0,1fr)_24rem]">
          <div className="min-w-0">
            <section aria-labelledby="about-event">
              ...
              <p className="mt-4 max-w-3xl whitespace-pre-line break-words text-base leading-8 text-slate-600 dark:text-slate-300">{event.description}</p>
            </section>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {event.organizer?.name ? (
                <section className="rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720] p-4" aria-labelledby="event-host">
                  <div className="flex items-center gap-3">
                    {event.organizer.avatar ? (
                      <img src={event.organizer.avatar} alt="" className="h-10 w-10 rounded-full border border-slate-200 dark:border-white/10 object-cover" />
                    ) : (
                      <span className="grid h-10 w-10 place-items-center rounded-full bg-teal-400/10 text-teal-700 dark:text-teal-300">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate font-semibold text-slate-900 dark:text-slate-100">{event.organizer.name}</p>
                    </div>
                  </div>
                </section>
              ) : null}
              ...
              {event.venue || event.city ? (
                <section className="rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720] p-4" aria-labelledby="event-venue">
                  <div className="flex items-start gap-3">
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-sky-400/10 text-sky-700 dark:text-sky-300">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 break-words font-semibold text-slate-900 dark:text-slate-100">{event.venue}</p>
                      {event.city ? <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{event.city}</p> : null}
                    </div>
                  </div>
                </section>
              ) : null}
...
function BackToEvents() {
  return (
    <Link to="/events" className="focus-ring inline-flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 transition hover:bg-slate-900/5 dark:hover:bg-white/[0.05] hover:text-teal-700 dark:hover:text-teal-200">
...
function EventInfoCard({ icon: Icon, label, value, detail, detailIcon: DetailIcon }) {
  ...
  return (
    <article className="flex items-start gap-3 rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720] p-3.5">
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-400/10 text-teal-700 dark:text-teal-300">
      ...
      <div className="min-w-0 pt-0.5">
        ...
        {detail ? (
          <p className="mt-1 flex items-center gap-1.5 break-words text-sm text-slate-600 dark:text-slate-400">
...
function EventDetailSkeleton() {
  return (
    <main className="min-h-[calc(100dvh-4.5rem)] bg-slate-50 dark:bg-[#080d14] text-slate-900 dark:text-white" aria-busy="true" aria-label="Loading event details">
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8 lg:px-8">
        <Skeleton className="h-9 w-32 !bg-slate-900/5 dark:!bg-white/5" />
        <Skeleton className="mt-5 aspect-[16/10] min-h-72 w-full !rounded-2xl !bg-slate-900/5 dark:!bg-white/5 sm:aspect-[16/8] lg:aspect-[16/7]" />
        <div className="mt-8 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_22rem] xl:grid-cols-[minmax(0,1fr)_24rem]">
          <div>
            <Skeleton className="h-8 w-52 !bg-slate-900/5 dark:!bg-white/5" />
            <Skeleton className="mt-4 h-24 w-full !bg-slate-900/5 dark:!bg-white/5" />
            <div className="mt-9 grid gap-3 sm:grid-cols-2">
              {Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-24 !rounded-xl !bg-slate-900/5 dark:!bg-white/5" />)}
            </div>
          </div>
          <Skeleton className="h-[27rem] !rounded-2xl !bg-slate-900/5 dark:!bg-white/5" />
        </div>
      </div>
    </main>
  );
