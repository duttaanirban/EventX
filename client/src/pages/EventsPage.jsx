    ...
    <main className="min-h-[calc(100dvh-4.5rem)] overflow-hidden bg-slate-50 dark:bg-[#080d14] text-slate-900 dark:text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8">
        <header className="mb-6 max-w-2xl">
          <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-teal-300/15 bg-teal-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-teal-700 dark:text-teal-200">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-400 sm:text-base">Find events that match your interests, location, and schedule.</p>
        </header>
          ...
          <div className="mt-4 flex flex-wrap gap-2" aria-label="Active filters">
            {activeChips.map((chip) => (
              <button
                ...
                className="focus-ring inline-flex max-w-full items-center gap-2 rounded-full border border-slate-200 dark:border-white/10 bg-slate-900/5 dark:bg-white/[0.05] px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 transition hover:border-teal-400/40 hover:bg-teal-400/10 hover:text-teal-700 dark:hover:text-teal-100"
              >
        ...
        <div className="mt-6 flex min-h-10 flex-wrap items-center justify-between gap-3 text-sm">
          ...
          {hasActiveFilters ? (
            <Button variant="nav" size="compact" onClick={clearFilters} className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-900/5 dark:bg-white/[0.03]">
        ...
        <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-busy={isLoading}>
          {isLoading
            ? Array.from({ length: 6 }).map((_, index) => (
              <div key={index} aria-hidden="true" className="overflow-hidden rounded-xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#101720]">
                <Skeleton className="aspect-video !rounded-none !bg-slate-900/5 dark:!bg-white/5" />
                <div className="space-y-4 p-4">
                  <Skeleton className="h-12 !bg-slate-900/5 dark:!bg-white/5" />
                  <Skeleton className="h-5 w-3/4 !bg-slate-900/5 dark:!bg-white/5" />
                  <Skeleton className="h-5 w-2/3 !bg-slate-900/5 dark:!bg-white/5" />
                  <Skeleton className="h-20 !bg-slate-900/5 dark:!bg-white/5" />
                </div>
              </div>
            ))
        ...
        {!isLoading && isError ? (
          <div role="alert" className="mt-6 rounded-2xl border border-rose-400/20 bg-rose-950/20 p-6 text-center text-sm text-rose-700 dark:text-rose-100">
        ...
        {data?.pages > 1 ? (
          <nav aria-label="Event pagination" className="mt-8 flex flex-wrap items-center justify-center gap-3">
            ...
            <span className="rounded-full border border-slate-200 dark:border-white/10 bg-slate-900/5 dark:bg-white/[0.04] px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400">Page {filters.page} of {data.pages}</span>
